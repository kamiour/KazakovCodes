...
$(container).mousemove(function (event) {
  for (let i = 0; i < heroTextarea.length; i++) {
    ...
    heroTextarea[i].style.transform = `perspective(450px) rotateX(${
      -(-startT + event.clientY) * 0.00012
    }rad) rotateY(${-(-startL + event.clientX) * 0.00005}rad)`;
    if (i == 0) {
      heroContacts[i].style.transitionDuration = "0s";
      heroContacts[i].style.transform = `perspective(450px) rotateX(${
        (-startT + event.clientY) * 0.00012
      }rad) rotateY(${(-startL + event.clientX) * 0.00005}rad)`;
    }
  }
